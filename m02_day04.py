...
try:
    ...
    matplotlib.rcParams['font.family'] = 'Dejavu Sans' # 운영체제가 맥, 리눅스인 경우
...
# ----------------------------------------------------
# 5. 컬럼 상수 정의 및 데이터 검증
# 상수로 분리하는 이유 : 컬럼명을 코드 여러 곳에 문자열로 쓰면 오타를 발견하기 어렵다.
...
with tab1:
    ...
    colors = ["#F36043", '#44F041', '#41B0F0', '#F759E0']
    # zip() : 튜플로 합치는 함수 --> 각각의 같은 인덱스 번호끼리 묶는다.
    for ax, col, color in zip(axes, df.columns, colors):
        # bins:구간, alpha:투명도(1이 100%), edgecolor:테두리 색
...
with tab2:
    ...
    with col1:
        st.markdown('**상관계수 행렬**')
        st.dataframe(corr.style.background_gradient(cmap='RdPu').format('{:.3f}'), # 배경색
                     width='stretch')
    with col2:
...
with tab3:
    ...
    r2_1 = r2_score(y_test1, y_pred1) # r2 스코어(결정계수) - 모델이 잘 만들어 졌는가?
    w1 = model1.coef_[0]
